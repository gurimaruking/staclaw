...
/**
 * Embedded 8x16 bitmap font for LCD text rendering.
 * Covers ASCII 0x20-0x7E (printable characters).
 * Each character is 16 bytes (1 byte per row, 16 rows).
...
#define UI_FONT_LAST   0x7E

/**
 * Font bitmap data: 95 characters x 16 bytes each.
 * Based on the classic VGA 8x16 font.
 */
static const uint8_t ui_font_data[][16] = {
...
/**
 * Get glyph data for a character. Returns NULL for out-of-range.
 */
static inline const uint8_t *ui_font_glyph(char c)
{
    if (c < UI_FONT_FIRST || c > UI_FONT_LAST) return ui_font_data[0]; // space
    return ui_font_data[c - UI_FONT_FIRST];
}
